...
DLLEXP unsigned char* h3dutGetFileData( void *hander, const char *filename, unsigned long *pSize)
{
    // 多次导入文件时候，重复利用buf
    ...
    *pSize = 0;
    ...
    do
    {
        ...
        *pSize = ftell(fp);
        ...
        if (*pSize > bufSize || NULL == FileBuffer)
        {
            delete[] FileBuffer; FileBuffer = NULL;
            FileBuffer = new unsigned char[*pSize];
            if (NULL == FileBuffer)
            {
                fclose(fp);
                *pSize = 0;
        ...
        *pSize = fread(FileBuffer, sizeof(unsigned char), *pSize, fp);
        fclose(fp);
    #elif defined PLATFORM_ANDROID
        AAssetManager* mgr = (AAssetManager*)hander;
        /*获取文件名并打开*/
        AAsset* asset = AAssetManager_open(mgr, filename, AASSET_MODE_UNKNOWN);
        if (asset == NULL)
        {
            //info_log("get assets file: [%s] failed", filename);
            return NULL;
        }
        /*获取文件大小*/
        *pSize = AAsset_getLength(asset);
        //info_log("file [%s] size : %ld\n", filename, *pSize);
        if (*pSize > bufSize || NULL == FileBuffer)
        {
            delete[] FileBuffer; FileBuffer = NULL;
            FileBuffer = new unsigned char[*pSize];
        ...
        *pSize = AAsset_read(asset, FileBuffer, *pSize);
        /*关闭文件*/
        AAsset_close(asset);
    #endif
    } while (0);
...
DLLEXP bool h3dutLoadResourcesFromDisk( void *contentDir, const char* resDir, const char* platformSubDir )
{
	bool result = true;
	string dir;
	vector< string > dirs;
	...
	while( res != 0 )
	{
        ...
        for( unsigned int i = 0; i < dirs.size(); ++i )
        {
            ...
            string resPathName = h3dGetResName(res);
            string resPath, resName;
            splitPath(resPathName, resPath, resName);
            
            allfiles.push_back(dirs[i] + resPathName);
            
            string prefix = "";
            if (resDir != NULL && *resDir != 0)
            {
                prefix = dirs[i] + resDir + "/";
                allfiles.push_back(prefix + resPath + resName);
            }
            string suffix = "";
            if (platformSubDir != NULL && *platformSubDir != 0)
            {
                suffix = string(platformSubDir) + "/";
                allfiles.push_back(dirs[i] + resPath + suffix + resName);
            }
            
            if (prefix != "" && suffix != "")
            {
                allfiles.push_back(prefix + resPath + suffix + resName);
            }
            ...
            for (int j = 0; j < allfiles.size(); ++j)
            {
            #ifdef PLATFORM_IOS
                inf.clear();
                inf.open( allfiles[j].c_str(), ios::binary );
                if(inf.good())
                {
            ...
            #elif defined PLATFORM_ANDROID
                //info_log("check file: [%s]", allfiles[j].c_str());
                AAssetManager* mgr = (AAssetManager*)contentDir;
                /*获取文件名并打开*/
                AAsset* asset = AAssetManager_open(mgr, allfiles[j].c_str(), AASSET_MODE_UNKNOWN);
                if (asset != NULL)
                {
                    fullFileName = allfiles[j];
                    AAsset_close(asset);
                ...
                //info_log("check file: [%s] failed", allfiles[j].c_str());
            #endif
            }
		...
		{
		    //info_log("load file: [%s]", fullFileName.c_str());
            dataBuf = h3dutGetFileData(contentDir, fullFileName.c_str(), &bufSize);
            if (dataBuf && bufSize)
            {
                // Send resource data to engine
                result &= h3dLoadResource( res, (const char*)dataBuf, (int)bufSize );
            }
            else
            {
