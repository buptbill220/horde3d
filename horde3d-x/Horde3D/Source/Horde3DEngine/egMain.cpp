...
DLLEXP void h3dShowBackground( const float *verts, const float *texCoords, int vertCount, uint32 backgroundRes)
{
    Resource *resObj = Modules::resMan().resolveResHandle( backgroundRes );
    APIFUNC_VALIDATE_RES_TYPE( resObj, ResourceTypes::Background, "h3dShowBackground", APIFUNC_RET_VOID );
    Modules::renderer().showBackground((const Vec3f*)verts, (const Vec3f*)texCoords, vertCount, (BackgroundResource*)resObj);
}

DLLEXP void h3dSetBackgroundTexture(const char* texturename, char* data, unsigned int size, int width, int height, int stride, uint32 backgroundRes)
{
    BackgroundResource *resObj = (BackgroundResource*)Modules::resMan().resolveResHandle( backgroundRes );
    APIFUNC_VALIDATE_RES_TYPE( resObj, ResourceTypes::Background, "h3dSetBackgroundTexture", APIFUNC_RET_VOID );
    resObj->setMemTexture(texturename, data, size, width, height, stride);
}
...
DLLEXP bool h3dLoadResource( ResHandle res, const char *data, int size )
{
    ...
    string filename = resObj->getName();
...
DLLEXP int h3dGetBindingFBO()
{
    return gRDI->getFBO();
}


DLLEXP void* h3dGenerateTexture( const char *name, int width, int height, int stride, int flags, const char* data)
{
    TextureResource *texRes = new TextureResource( safeStr( name, 0 ), flags );
    ResHandle res = Modules::resMan().addNonExistingResource( *texRes, true );
    if( res == 0 )
    {
        Modules::log().writeDebugInfo( "Failed to add resource in h3dGenTexture; maybe the name is already in use?", res );
        delete texRes;
        return NULL;
    }
    if (!texRes->load(data, width * height * stride, height, width, stride)){
        Modules::log().writeDebugInfo( "Failed to load texture in h3dGenTexture", res );
        delete texRes;
        return NULL;
    }
    return texRes;
