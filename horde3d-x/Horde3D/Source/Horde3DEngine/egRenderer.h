...
struct BackgroundVert
{
    ...
    float u, v;  //Texure coordinates
};

struct BackgroundBatch
{
    PBackgroundResource materialRes;
    uint32              firstVert, vertCount;
    BackgroundBatch(uint32 firstVert, uint32 vertCount, BackgroundResource *materialRes) :
        firstVert(firstVert), vertCount(vertCount), materialRes(materialRes) {}
    BackgroundBatch() : firstVert(0), vertCount(0), materialRes(0x0) {}
};
...
public:
	...
	void clearOverlays();
    void showBackground( const Vec3f *vertPoss, const Vec3f *texCoords, uint32 vertCount, BackgroundResource *matRes);
...
protected:
    ...
    void drawBackground( const std::string &shaderContext, const std::string &theClass, RenderingOrder::List order );
...
protected:
    ...
    BackgroundBatch                    _backgroundBatch;
    uint32                             _backgroundVB;
    BackgroundVert                     *_backgroundVerts;
	...
	uint32                             _vlPosOnly, _vlOverlay, _vlModel, _vlParticle, _vlBackground;
