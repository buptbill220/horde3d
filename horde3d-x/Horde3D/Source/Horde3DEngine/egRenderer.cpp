...
bool Renderer::init()
{
    ...
    VertexLayoutAttrib attribsBackground[2] = {
        {"vertPos", 0, 3, 0},
        {"texCoords0", 0, 2, 12}
    };
    _vlBackground = gRDI->registerVertexLayout(2, attribsBackground);
    ...
    _backgroundVerts = new BackgroundVert[MaxNumOverlayVerts];
    _backgroundVB = gRDI->createVertexBuffer( MaxNumOverlayVerts * sizeof(BackgroundVert), 0x0 );
...
void Renderer::showBackground( const Vec3f *vertPos, const Vec3f *texCoords, uint32 vertCount, BackgroundResource *matRes)
{
    _backgroundBatch.firstVert = 0;
    _backgroundBatch.vertCount = vertCount;
    _backgroundBatch.materialRes = matRes;
    
    char* p = (char*)_backgroundVerts;
    for (uint32 i = 0; i < vertCount; ++i)
    {
        memcpy(p, vertPos + i, sizeof(Vec3f));
        p += sizeof(Vec3f);
        memcpy(p, texCoords + i, sizeof(float) * 2);
        p += sizeof(float) * 2;
    }
}

void Renderer::drawBackground( const std::string &shaderContext, const std::string &theClass, RenderingOrder::List order)
{
    if (_backgroundBatch.materialRes == 0x0)
    {
        BackgroundResource* matRes = (BackgroundResource*)Modules::resMan().findResource(ResourceTypes::Background, "");
        if (matRes == 0x0)  return ;
        BackSampler& sampler = matRes->_samplers.back();
        showBackground(sampler.vertPos, sampler.texCoord, sampler.vertSize, matRes);
    }
        
    // Upload overlay vertices
    gRDI->updateBufferData( _backgroundVB, 0, MaxNumOverlayVerts * sizeof( BackgroundVert ), _backgroundVerts );
        
    gRDI->setVertexBuffer( 0, _backgroundVB, 0, sizeof( BackgroundVert ) );
    ...
    {
        PShaderResource shaderRes = _backgroundBatch.materialRes->getShader();
        PBackgroundResource materialRes = _backgroundBatch.materialRes;
        {
            shaderRes = materialRes->_shaderRes;
            if( shaderRes == 0x0 ) return ;
            
            // Find context
            ShaderContext *context = shaderRes->findContext( shaderContext );
            if( context == 0x0 ) return ;
            
            // Set shader combination
            ShaderCombination *sc = shaderRes->getCombination( *context, materialRes->_combMask );
            if( sc != _curShader ) setShaderComb( sc );
            if( _curShader == 0x0 || gRDI->_curShaderId == 0 ) return ;
        ...
        for( size_t i = 0, si = shaderRes->_samplers.size(); i < si; ++i )
        {
            ...
            for( size_t j = 0, sj = materialRes->_samplers.size(); j < sj; ++j )
            {
                ...
                {
                    BackTexture texture;
                    if (materialRes->getTexture(materialRes->_samplers[j].name, texture))
                    {
                        texRes = texture.texRes;
                    }
                    break;
                }
            }
    ...
    gRDI->setVertexLayout( _vlBackground );
    
    // Draw batch
    gRDI->drawIndexed( PRIM_TRILIST, _backgroundBatch.firstVert * 6/4, _backgroundBatch.vertCount * 6/4, _backgroundBatch.firstVert, _backgroundBatch.vertCount );
}
...
void Renderer::render( CameraNode *camNode )
{
	...
	for( uint32 i = 0; i < _curCamera->_pipelineRes->_stages.size(); ++i )
	{
		...
		for( uint32 j = 0; j < stage.commands.size(); ++j )
		{
            ...
            case PipelineCommands::DrawBackground:
                drawBackground(pc.params[0].getString(), pc.params[1].getString(),  (RenderingOrder::List)pc.params[2].getInt());
                break;
