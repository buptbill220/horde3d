...
bool TextureResource::load(const char* data, uint32_t size, uint32_t width, uint32_t height, uint32_t offset)
{
    if( !Resource::load( data, size ) ) return false;
#define MAX_WIDTH   2048
#define MAX_HEIGHT  2048
    //load rgba texture to gles
    _width = width;
    _height = height;
    _depth = 1;
    _texType = TextureTypes::Tex2D;
    ...
    if (offset == 4) {
        ...
        if (s_pixels == NULL) {
            s_pixels = new char[2048 * 2048 * 4];
            if (s_pixels == NULL) {
                return false;
            }
        }
        char* p = (char*)s_pixels;
        for (uint32_t i = 0; i < size; i += 3) {
            memcpy((void*)p, data, 3);
            p += 3;
            *(uint8_t*)p = 0x80;
            ++p;
        }
    ...
    _texObject = gRDI->createTexture( _texType, _width, _height, _depth, _texFormat, _hasMipMaps, _hasMipMaps, _sRGB );
    gRDI->uploadTextureData( _texObject, 0, 0, pixels );
