...
const string PipelineResource::parseStage( XMLNode &node, PipelineStage &stage )
{
	...
	while( !node1.isEmpty() )
	{
        ...
        else if( strcmp( node1.getName(), "DrawBackground") == 0 )
        {
            //<DrawBackground context="GENERAL" class="BACK_TO_FRONT"/>
            if( !node1.getAttribute( "context" ) ) return "Missing DrawBackground attribute 'context'";
            ...
            stage.commands.push_back( PipelineCommand( PipelineCommands::DrawBackground ) );
            vector< PipeCmdParam > &params = stage.commands.back().params;
            params.resize( 3 );
            params[0].setString( node1.getAttribute( "context" ) );
            params[1].setString( node1.getAttribute( "class", "" ) );
