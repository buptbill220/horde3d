...
bool Converter::writeSceneGraph( const string &assetPath, const string &assetName )
{
    ...
    // default is model.scene.xml
    outf.open( (_outPath + assetPath + "model.scene.xml").c_str(), ios::out );
	if( !outf.good() )
	{
	...
	for( unsigned int i = 0; i < _daeDoc.libMaterials.materials.size(); ++i )
	{
		...
		if( material.effect != 0x0 )
		{
			...
			if( material.effect->diffuseMap != 0x0 )
			{
				...
				outf << assetPath << material.effect->diffuseMap->filePath << material.effect->diffuseMap->fileName;
				outf << "\" />\n";
			}
...
bool Converter::writeAnimation( const string &assetPath, const string &assetName )
{
    ...
    // default is model.anim
    FILE *f = fopen( (_outPath + assetPath + "model.anim").c_str(), "wb" );
	if( f == 0x0 )
	{
